...
PHYS_PROP_NAME = ADDON_PFX + "PHYSICS"
CHILD_PROP_NAME = ADDON_PFX + "CHILD"
...
class LODSections(types.Operator):
	...
	def check_subdirectories(self, context: types.Context):
		input = str(self.prop_custom_subdirectories)
		...
		if self.prop_custom_subdirectories != subdirectories:
			self.prop_custom_subdirectories = subdirectories
			LODSections.redraw = True

	def check_prefix(self, context: types.Context):
		prefix = ut.sanitized(self.prop_custom_prefix) if self.prop_custom_prefix else PFX
		if self.prop_custom_prefix != prefix:
			self.prop_custom_prefix = prefix
			LODSections.redraw = True
	...
	prop_use_custom_subdirectories = bpy.props.BoolProperty(
		name="Subdirectories",
		description="Use custom subdirectories",
		default=False
	)
	prop_custom_subdirectories = bpy.props.StringProperty(
		name="",
		default=SECT_PROP_NAME,
		description=(
			"The full path of all subdirectories. If any do not exist, they will be created automatically.\n"
			"Example: 'data/lod_sections'"
		),
	...
	def invoke(self, context: types.Context, event: types.Event) -> Set[str]:
		...
		for ob in context.selected_editable_objects:
			if isinstance(ob.data, types.Mesh):
				objects.append(ob)
		...
		if objects:
			...
			if isinstance(self.active_object.data, types.Mesh):
		...
		return context.window_manager.invoke_props_dialog(self, width=system_dpi * 8)
	...
	def draw(self, context: types.Context):
		...
		if SECT_PROP_NAME in self.active_object.game.properties:
		...
		col = row().column
		col().prop(self, "prop_use_custom_subdirectories", toggle=True)
		col_pref = col()
		col_pref.prop(self, "prop_custom_subdirectories")
		if not self.prop_use_custom_subdirectories:
			col_pref.active = False
			
	def check(self, context: types.Context) -> bool:
		if LODSections.redraw:
			LODSections.redraw = False
			return True
	...
	def execute(self, context: types.Context) -> Set[str]:
		...
		def store_initial_state():
			...
			pr.print_delta_timed("Storing initial state")
		...
		def create_bases():
			...
			for child in self.active_object.children:
				if CHILD_PROP_NAME in child.game.properties:
			...
			pr.print_delta_timed("Creating bases")
			...
			for ob in self.objects:
				...
				for mod in base.modifiers:
					...
					pr.print_delta_timed("Applying", mod.name)
			...
			pr.print_delta_timed("Cleaning up bases")
		...
		def dissolve_bases():
			for base in self.bases:
				...
				if self.prop_use_decimate_dissolve:
					...
					pr.print_delta_timed("Decimating base:", base.name)
		...
		def collect_data():
			...
			pr.print_delta_timed("Collecting data")

			for base in self.bases:
				self.materials.update(set(base.data.materials))

			self.dimensions = ut.dimensions(*self.bases, include_transform=True)
		...
		def generate_sections():
			...
			pr.print_delta_timed("Multisecting bases")
			...
			pr.print_delta_timed("Separating bases")
			...
			pr.print_delta_timed("Joining into sections")

			physics_type = "NO_COLLISION" if self.prop_use_lod and self.prop_lod_use_physics else self.physics_type
			for id, l in tmps_data.items():
				...
				sect.game.physics_type = physics_type
		...
		def generate_lod():
			...
			for i in range(1, self.prop_lod_number + 1):
				...
				pr.print_delta_timed("Generating LOD", i, "of", self.prop_lod_number)
			...
			pr.print_delta_timed("Configuring LOD")
			...
			self.lod_distances = [v - lod_dist_init for v in ut.get_table_incremented(lod_dist_init, lod_dist_fact, self.prop_lod_number + 1)]
			...
			for id, l in lod.items():
				...
				for i, sect_lod in enumerate(l):
					...
					lod_level.distance = self.lod_distances[i]
		...
		def collect_particles():
			for i, ob in enumerate(self.objects):
				...
				for psm in psml:
					...
					pr.print_delta_timed("Converting", psm.name)
		...
		def generate_lod_physics():
			for n in self.lod_tmps:
				...
				if n + PHYS not in self.scene.objects:
					pr.print_delta_timed("Generating physics for", n)
		...
		def copy_normals():
			...
			pr.print_delta_timed("Copying custom normals")
		...
		def generate_lod_materials():
			...
			pr.print_delta_timed("Generating lod materials")
		...
		def export_data():
			...
			pr.print_delta_timed("Exporting data")
			...
			data = {
				...
				HAS_PHYSICS : self.prop_lod_use_physics,
			...
			name = self.active_object.name
			dirs = Path(self.prop_custom_subdirectories)
			relp = os.path.join(str(dirs), name)
			absp = os.path.join(str(dirs.absolute()), name)

			pr.print_delta_timed("Writing compressed file: '//{}.zz'".format(relp))

			ut.write_zlibbed(absp, data)
		...
		def generate_physics():
			...
			pr.print_delta_timed("Generating Physics")
		...
		def finalize():
			...
			pr.print_delta_timed("Finalizing sections and generating bounds")
			...
			n = self.prefix + self.active_object.name + BOUNDS
			bounds.name = n
			bounds.data.name = n
			bounds.parent = self.sections
			bounds.select = False

			self.sections.select = True
		...
		def generate_game_logic():
			...
			pr.print_delta_timed("Generating game logic")

			self.scene.objects.active = self.active_object
			self.active_object.select = True

			old = "# {}".format(TO_BE_REPLACED)
			new = ut.get_json(TO_BE_REPLACED, str(Path(self.prop_custom_subdirectories)))
			ut.add_text(self.bl_idname, True, TOOL_NAME, replace=[old, new])

			ut.add_game_property(self.active_object, PHYS_PROP_NAME, self.physics_type)
			ut.add_game_property(self.active_object, SECT_PROP_NAME, self.sections.name)
			ut.add_game_property(self.active_object, PROG_PROP_NAME, 0.0, True)
			ut.add_logic_python(self.active_object, TOOL_NAME, "__init__", False, 1, 30)
			ut.add_logic_python(self.active_object, TOOL_NAME, "init", False, 2)
			ut.add_logic_python(self.active_object, TOOL_NAME, "load", False, 3, 30)
			ut.add_logic_python(self.active_object, TOOL_NAME, "edit", True, 4)
			ut.add_logic_python(self.active_object, TOOL_NAME, "add", True, 5)
			ut.add_logic_python(self.active_object, TOOL_NAME, "populate", True, 6)
			ut.add_logic_python(self.active_object, TOOL_NAME, "settle", True, 7, 60)
			ut.add_logic_python(self.active_object, TOOL_NAME, "update", True, 8)
			ut.add_logic_python(self.active_object, TOOL_NAME, "end", False, 9)

			self.active_object.game.use_all_states = True

		def restore_initial_state():

			pr.print_delta_timed("Restoring initial state")
		...
		def update_log_msg():
			self.log_msg = "Finished generating {} ({} X {}) sections".format(len(self.data), round(self.size.x, 1), round(self.size.y, 1))

			pr.print_timed(self.log_msg)
			pr.reset(False)
		...
		pr.print_timed("Start executing {}".format(self.bl_idname))

		self.prefix = self.prop_custom_prefix if self.prop_use_custom_prefix else PFX

		props = self.active_object.game.properties
		if SECT_PROP_NAME in props:

			if self.prop_update_or_clear == "update":
				self.err_msg = "Update: not implemented yet"
				return {"CANCELLED"}

			physics_type = props[PHYS_PROP_NAME].value if PHYS_PROP_NAME in props else ""
			sections_name = props[SECT_PROP_NAME].value

			try:
				ut.remove_game_properties(self.active_object, [PHYS_PROP_NAME, SECT_PROP_NAME, PROG_PROP_NAME])
				...
				if physics_type:
					self.active_object.game.physics_type = physics_type

				pr.print_timed("Finished clearing existing lod sections")
