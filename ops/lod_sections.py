...
class LODSections(bpy.types.Operator):
	...
	def execute(self, context):
		...
		def create_bases():
			...
			selected_and_children.update(self.active_object.children)
			bpy.ops.object.select_all(action="DESELECT")
			
			for ob in selected_and_children:
				transform_inverted = self.active_object.matrix_world.inverted()
				...
				if pl:
					for p in pl:
						...
						if n not in self.lod_tmps:
							self.lod_tmps[n] = []
						m = transform_inverted * ob.matrix_world
			...
			for ob in self.objects:
				...
				bases.extend(context.selected_objects)
				bpy.ops.object.select_all(action="DESELECT")
			...
			for base in bases:
				...
				self.transforms.append(base.matrix_world.copy())
		...
		def dissolve_bases():
			...
			print(self.profiler.timed("Decimating bases"))
			...
			for base in self.bases:
				...
				bpy.ops.object.editmode_toggle()
				bpy.ops.mesh.select_all(action="SELECT")
				bpy.ops.mesh.dissolve_limited(angle_limit=self.prop_decimate_dissolve_angle_limit, delimit={"NORMAL", "MATERIAL", "SEAM", "SHARP", "UV"})
				bpy.ops.mesh.select_all(action="SELECT")
				bpy.ops.mesh.quads_convert_to_tris()
				bpy.ops.mesh.beautify_fill()
				bpy.ops.mesh.tris_convert_to_quads()
		...
		def generate_lod():
			...
			if self.prop_lod_use_distance:
				...
				lod_dist = int(1.25 * math.sqrt(self.size.x * self.size.y))
		...
		def collect_particles():
			for i, ob in enumerate(self.objects):
				...
				for psm in psml:
					...
					transform_inverted = self.transforms[i].inverted()
					for o in selected_objects:
						m = transform_inverted * o.matrix_world
						
						if settings.dupli_object.lod_levels:
							n = settings.dupli_object.name
							if n not in self.lod_tmps:
								self.lod_tmps[n] = []
							self.lod_tmps[n].append([m, {}])
							ut.remove(o, False)
							continue
		...
		def generate_game_logic():
			...
			ut.add_logic_python(self.active_object, TOOL_NAME, "init")
