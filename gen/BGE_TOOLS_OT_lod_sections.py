...
class LODSections(types.KX_GameObject):
	...
	def __init__(self, own):
		self.visible = False
		self.reinstancePhysicsMesh(None, None)
		for ob in self.children:
			if ob.groupMembers is None:
				ob.endObject()
	...
	def load(self):
		...
		def get_data():
			if not (LOD_SECTIONS_PROP_NAME in self and self[LOD_SECTIONS_PROP_NAME]):
				return
		...
		for ob in self.children:
			if ob.groupMembers is not None:
				for o in ob.groupMembers:
					if o.parent is None:
						self.add_instance(o, ob.worldTransform, {n : ob[n] for n in ob.getPropertyNames()})
				ob.endObject()
			...
			if sect not in self.physical_sections:
				...
				for n, nl in self.instances[sect].items():
					...
					for l in nl:
						...
						m = Matrix()
						m[:] = l[0]
						o.setParent(inst, True, False)
						o.worldTransform = self.localTransform * m
			...
			if sect not in self.visual_sections:
				...
				for n, nl in self.instances[sect].items():
					for l in nl:
						...
						if gpl is not None:
							for gp in gpl:
								gp.worldTransform = self.localTransform * m
						else:
...
def init(cont):
	if not cont.sensors[0].positive:
		return
	obj = cont.owner
	lod_sections = LODSections(obj)
	assert(obj is not lod_sections)
	assert(obj.invalid)
	assert(lod_sections is cont.owner)
